...
function oscan::log::errexit() {
	...
	set +o xtrace
	local code="${1:-1}"
	oscan::log::error_exit "'${BASH_COMMAND}' exited with status $err" "${1:-1}" 1
}

function oscan::log::install_errexit() {
	# trap ERR to provide an error handler whenever a command exits nonzero this
	# is a more verbose version of set -o errexit
	trap 'oscan::log::errexit' ERR
...
function oscan::log::error_exit() {
	...
	oscan::log::stack $stack_skip

	echo "Exiting with status ${code}" >&2
	exit "${code}"
}

function oscan::log::with-severity() {
  local msg=$1
  local severity=$2
...
function oscan::log::info() {
  oscan::log::with-severity "${1}" "INFO"
}

function oscan::log::warn() {
  oscan::log::with-severity "${1}" "WARNING"
}

function oscan::log::error() {
  oscan::log::with-severity "${1}" "ERROR"
}
...
function oscan::util::host_platform() {
  echo "$(go env GOHOSTOS)/$(go env GOHOSTARCH)"
}

function oscan::util::sed() {
...
function oscan::util::base64decode() {
  if [[ "$(go env GOHOSTOS)" == "darwin" ]]; then
  	base64 -D $@
  else
  	base64 -d $@
...
function oscan::util::get_object_assert() {
  local object=$1
  local request=$2
  local expected=$3
